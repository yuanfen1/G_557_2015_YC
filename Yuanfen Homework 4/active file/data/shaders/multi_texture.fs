...
uniform sampler2D texture_color; //this is the texture
...
void main(void)                                                   
{
    ...
    vec4 tex_color_colormap =  texture(texture_color, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + tex_color_colormap;
// + tex_color_light+ tex_color + 
    }
    else if(texture_blend == 1)
    {
        color = tex_color * tex_color_light * tex_color_colormap;
    }
    else if(texture_blend == 2)
    {
        color = (tex_color_light.r)*tex_color *  tex_color_colormap;
    }
    else
    {
        color = 0.1 * pass_Color + tex_color + tex_color_colormap;
    }
