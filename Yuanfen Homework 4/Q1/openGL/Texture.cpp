...
string  GLMultiTexture::_glsl_names[4] = { "texture_background", "texture_foreground", "texture_middle", "texture_blend"};
#endif

GLTexture::GLTexture()
{
    _textureIdx = -1;
    _texture_blend_mode = 0;//0
    _dirty = false;
}
...
int GLMultiTexture::loadAndCreateTextures(string path_and_file_texture_1, string path_and_file_texture_2, string path_and_file_texture_3)
{
    ...
    //**********************************************************************************************
    // Texture generation - landscape (texture 1)
    ...
    free( data2 );
	 ...
	 //**********************************************************************************************
    // Texture generation - color pattern (texture 3)
    
    // Activate a texture unit
    glActiveTexture(GL_TEXTURE2);//GL_TEXTURE strat from 0
...
bool GLMultiTexture::addVariablesToProgram(GLuint program, int variable_index)
{
    ...
    //****************************************************************************************************
    // Link the texture to the uniform variable and texture unit 0;  texture 1
    ...
    /*
     Then we set the tex uniform of the shaders to the index of the texture unit. We used texture unit zero, so we set the tex uniform to the integer value 0.
     */
    glUniform1i(_textureIdx3, 2);    //was 1 and false
