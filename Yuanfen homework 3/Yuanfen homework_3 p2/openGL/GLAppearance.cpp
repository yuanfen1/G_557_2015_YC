...
bool GLSpotLightSource::addVariablesToProgram(GLuint program, int variable_index)
{
	...
	_smooth_angleIdx = glGetUniformLocation(program, GetVariableName(_glsl_object, _glsl_names[2], variable_index).c_str());
                    checkUniform(_smooth_angleIdx, GetVariableName(_glsl_object, _glsl_names[2], variable_index));
	...
	glUniform1f(_smooth_angleIdx, _smooth_angle);
